...
class KGMLP(nn.Module):
    def __init__(self, model_name, nentity, nrelation, nitem, nuser, hidden_dim, gamma, mlp_layers=[16, 8],
                 double_entity_embedding=False, double_relation_embedding=False, dropout=False):
        super(KGMLP, self).__init__()
        ...
        # MLP architecture
        assert (mlp_layers[0] % 2 == 0), "layers[0] must be an even number"
        self.__alias__ = "MLP {}".format(mlp_layers)
        ...
        # hidden dense layers
        for _, (in_size, out_size) in enumerate(zip(mlp_layers[:-1], mlp_layers[1:])):
    ...
    def train_step(model, optimizer, kg_train_iterator, args):
        ...
        positive_sample, negative_sample, subsampling_weight, mode = next(kg_train_iterator)
